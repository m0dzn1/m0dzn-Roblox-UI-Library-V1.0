...
-------------------------------------------------------------------------------------
-- LIQUID GLASS CONFIGURATION (replaces old Themes system)
-------------------------------------------------------------------------------------

local LiquidGlassConfig = {
    -- Reflection: Part.Transparency glitched above 1 + invisible Highlight
    ReflectionEnabled    = true,
    ReflectionIntensity  = 5,               -- Glass Part Transparency (1.5–15, higher = more chrome)
    EnvironmentSpecular  = 1,               -- Lighting.EnvironmentSpecularScale (0–1)

    -- Surface Blur & Tint
    GlassOpacity         = 0.55,            -- Panel BackgroundTransparency (0 = solid, 1 = invisible)
    GlassTint            = Color3.fromRGB(220, 240, 255), -- Ice-blue liquid tint
    RefractionIntensity  = 0.15,            -- Border glow / UIStroke edge strength (0–1)

    -- Depth Blur (DepthOfFieldEffect in Lighting)
    DepthBlurEnabled     = true,
    DepthBlurIntensity   = 0.8,             -- DepthOfField FarIntensity / NearIntensity (0–1)
    FocusRadius          = 15,              -- Sharp focus zone around the UI part (studs)

    -- Liquid Wave Animation
    LiquidSpeed          = 2.5,             -- Seconds per sheen sweep cycle
    LiquidGlossOpacity   = 0.5,             -- Peak brightness of the moving gloss band (0–1)
    SheenColor           = Color3.fromRGB(255, 255, 255),
}

-------------------------------------------------------------------------------------
-- DERIVE UI COLORS FROM GLASS TINT
-------------------------------------------------------------------------------------

local function DeriveGlassColors()
    local tint = LiquidGlassConfig.GlassTint
    local h, s, v = Color3.toHSV(tint)

    return {
        -- Deep dark base with hint of tint — readable over any game scene
        Main   = Color3.fromHSV(h, math.clamp(s * 0.45, 0, 1), 0.11),
        -- Slightly lighter for tiles, buttons, notifications
        Top    = Color3.fromHSV(h, math.clamp(s * 0.38, 0, 1), 0.17),
        -- Near-white with faint tint for readability
        Text   = Color3.fromHSV(h, math.clamp(s * 0.08, 0, 0.12), 1),
        -- Vivid saturated accent from the tint hue
        Accent = Color3.fromHSV(h, math.max(s, 0.5), 1),
        -- Muted stroke for borders
        Stroke = Color3.fromHSV(h, math.clamp(s * 0.35, 0, 1), 0.32),
    }
end

local CurrentColors = DeriveGlassColors()

-------------------------------------------------------------------------------------
-- REGISTRY SYSTEM
-------------------------------------------------------------------------------------

local function AddToRegistry(obj, prop, colorKey)
    table.insert(Registry, {Object = obj, Property = prop, Type = colorKey})
    obj[prop] = CurrentColors[colorKey]
end

-- Transparency registry: modifier is added to GlassOpacity for this element
local function AddGlassTransparency(obj, modifier)
    table.insert(TransRegistry, {Object = obj, Modifier = modifier or 0})
    obj.BackgroundTransparency = math.clamp(LiquidGlassConfig.GlassOpacity + modifier, 0, 0.98)
end
...
function Library:CreateWindow(Config)
    ...
    -- Allow initial glass config override from CreateWindow call
    if Config.LiquidGlass and type(Config.LiquidGlass) == "table" then
        for key, val in pairs(Config.LiquidGlass) do
            if LiquidGlassConfig[key] ~= nil then
                LiquidGlassConfig[key] = val
            end
        end
        CurrentColors = DeriveGlassColors()
    end
    ...
    -- Save original lighting state for cleanup
    ...
    ---------------------------------------------------------------------------------
    -- MAIN FRAME (CanvasGroup for proper sheen clipping)
    ---------------------------------------------------------------------------------
    ...
    AddGlassTransparency(MainFrame, 0)

    -- Glass depth curvature gradient (diagonal fade gives curved capsule illusion)
    local GlassGradient = Instance.new("UIGradient")
    GlassGradient.Rotation = 45
    GlassGradient.Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0, math.clamp(LiquidGlassConfig.GlassOpacity - 0.12, 0, 1)),
        NumberSequenceKeypoint.new(1, math.clamp(LiquidGlassConfig.GlassOpacity + 0.18, 0, 1))
    })
    GlassGradient.Parent = MainFrame

    -- Refraction edge glow stroke
    local RainbowThickness = 2
    local Stroke = Instance.new("UIStroke")
    Stroke.Thickness = 1.5
    Stroke.Transparency = 1 - LiquidGlassConfig.RefractionIntensity
    ...
    ---------------------------------------------------------------------------------
    -- LIQUID SHEEN OVERLAY (animated gloss sweep)
    ---------------------------------------------------------------------------------

    local Sheen = Instance.new("Frame")
    Sheen.Name = "LiquidSheen"
    Sheen.Size = UDim2.new(0.4, 0, 2, 0)
    Sheen.Position = UDim2.new(-0.5, 0, -0.5, 0)
    Sheen.BackgroundColor3 = LiquidGlassConfig.SheenColor
    ...
    SheenGradient.Rotation = 70
    SheenGradient.Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0, 1),
        NumberSequenceKeypoint.new(0.5, 1 - LiquidGlassConfig.LiquidGlossOpacity),
        NumberSequenceKeypoint.new(1, 1)
    })
    SheenGradient.Parent = Sheen

    local sheenTween = nil

    local function AnimateSheen()
        if sheenTween then pcall(function() sheenTween:Cancel() end) end
        Sheen.Position = UDim2.new(-0.5, 0, -0.5, 0)
        local info = TweenInfo.new(
            LiquidGlassConfig.LiquidSpeed,
            Enum.EasingStyle.Linear,
            Enum.EasingDirection.InOut,
            -1, false
        )
        sheenTween = TweenService:Create(Sheen, info, {Position = UDim2.new(1.3, 0, -0.5, 0)})
        sheenTween:Play()
    end
    task.spawn(AnimateSheen)

    ---------------------------------------------------------------------------------
    -- PHYSICAL GLASS PART (Reflection via Transparency > 1 + Highlight)
    ---------------------------------------------------------------------------------

    local glassPart = nil
    local glassFolder = nil
    local glassTrackConn = nil
    ...
    local function CreateGlassPart()
        ...
        glassFolder = Instance.new("Folder")
        glassFolder.Name = "M0dzn_GlassReflector"
        glassFolder.Parent = Workspace

        glassPart = Instance.new("Part")
        glassPart.Name = "GlassPane"
        glassPart.Size = Vector3.new(18, 12, 0.1)
        ...
        glassPart.Color = LiquidGlassConfig.GlassTint
        glassPart.Transparency = LiquidGlassConfig.ReflectionIntensity
        glassPart.Parent = glassFolder

        -- Invisible Highlight keeps engine refraction calculations active
        local hl = Instance.new("Highlight")
        hl.Name = "GlassReflectHook"
        hl.FillTransparency = 1
        hl.OutlineTransparency = 1
        hl.Parent = glassPart

        -- Apply specular
        Lighting.EnvironmentSpecularScale = LiquidGlassConfig.EnvironmentSpecular

        -- Track camera position — keep the glass pane in front of the camera
        glassTrackConn = RunService.RenderStepped:Connect(function()
            local c = Workspace.CurrentCamera
            if c and glassPart and glassPart.Parent then
                glassPart.CFrame = c.CFrame * CFrame.new(0, 0, -8)
            end
        end)
    end

    local function DestroyGlassPart()
        if glassTrackConn then glassTrackConn:Disconnect(); glassTrackConn = nil end
        if glassFolder then pcall(function() glassFolder:Destroy() end); glassFolder = nil end
        glassPart = nil
        Lighting.EnvironmentSpecularScale = origSpecular
    end

    local function UpdateGlassPart()
        if not LiquidGlassConfig.ReflectionEnabled then
            DestroyGlassPart()
            return
        end
    ...
    -- Initial glass part
    if LiquidGlassConfig.ReflectionEnabled then
        task.defer(CreateGlassPart)
    end
    ...
    local dof = nil
    local dofTrackConn = nil

    local function UpdateDepthOfField()
        if not LiquidGlassConfig.DepthBlurEnabled then
            if dof then pcall(function() dof:Destroy() end); dof = nil end
            if dofTrackConn then dofTrackConn:Disconnect(); dofTrackConn = nil end
            return
        end

        if not dof then
            dof = Lighting:FindFirstChild("M0dzn_DepthBlur")
            if not dof then
                dof = Instance.new("DepthOfFieldEffect")
                dof.Name = "M0dzn_DepthBlur"
                dof.Parent = Lighting
            end
        end

        dof.FarIntensity = LiquidGlassConfig.DepthBlurIntensity
        dof.NearIntensity = LiquidGlassConfig.DepthBlurIntensity
        dof.InFocusRadius = LiquidGlassConfig.FocusRadius

        -- Track camera distance to the glass pane for dynamic focus
        if not dofTrackConn then
            dofTrackConn = RunService.Heartbeat:Connect(function()
                if dof and dof.Parent and glassPart and glassPart.Parent then
                    local cam = Workspace.CurrentCamera
                    if cam then
                        local dist = (cam.CFrame.Position - glassPart.Position).Magnitude
    ...
    task.defer(UpdateDepthOfField)

    ---------------------------------------------------------------------------------
    -- APPLY GLASS CHANGES (master update — called by all settings callbacks)
    ---------------------------------------------------------------------------------

    local function ApplyGlassChanges()
        CurrentColors = DeriveGlassColors()

        -- Update color registry
        for _, r in pairs(Registry) do
            if r.Object and r.Object.Parent then
                Tween(r.Object, {[r.Property] = CurrentColors[r.Type]})
            end
        end

        -- Update transparency registry
        for _, t in pairs(TransRegistry) do
            if t.Object and t.Object.Parent then
                Tween(t.Object, {BackgroundTransparency = math.clamp(LiquidGlassConfig.GlassOpacity + t.Modifier, 0, 0.98)})
            end
        end

        -- Glass gradient depth curve
        GlassGradient.Transparency = NumberSequence.new({
            NumberSequenceKeypoint.new(0, math.clamp(LiquidGlassConfig.GlassOpacity - 0.12, 0, 1)),
            NumberSequenceKeypoint.new(1, math.clamp(LiquidGlassConfig.GlassOpacity + 0.18, 0, 1))
        })
    ...
    AddGlassTransparency(ProfileFrame, 0.04)
    ...
    PStroke.Thickness = 1; PStroke.Transparency = 0.75; PStroke.Parent = ProfileFrame
    ...
    DispName.Font = Enum.Font.GothamBold; DispName.TextSize = 11
    ...
    UsrName.Font = Enum.Font.Gotham; UsrName.TextSize = 10
    ...
    local function SpawnNotif(title, body, notifType, source)
        task.spawn(function()
            ...
            NTimer.Font = Enum.Font.GothamBold; NTimer.TextSize = 11
            ...
            NText.Font = Enum.Font.GothamMedium; NText.TextSize = 11
            ...
            NBadge.Parent = Notif
            NBadge.Font = Enum.Font.GothamBold; NBadge.TextSize = 9
    ...
    function Window:Unload()
        ...
        Dialog.BackgroundColor3 = CurrentColors.Main
        Dialog.ZIndex = 501
        ...
        DStroke.Color = CurrentColors.Stroke
        ...
        DAccentBar.BackgroundColor3 = CurrentColors.Accent
        ...
        DTitleDot.BackgroundColor3 = CurrentColors.Accent
        ...
        DTitle.TextColor3 = CurrentColors.Text
        DTitle.ZIndex = 502
        ...
        DBody.Font = Enum.Font.GothamMedium; DBody.TextSize = 12
        DBody.TextXAlignment = Enum.TextXAlignment.Left
        DBody.TextWrapped = true
        DBody.TextColor3 = CurrentColors.Text
        ...
        DDivider.BackgroundColor3 = CurrentColors.Stroke
        ...
        NoBtn.Text = "No"
        NoBtn.Font = Enum.Font.GothamBold; NoBtn.TextSize = 13
        NoBtn.TextColor3 = CurrentColors.Main
        NoBtn.BackgroundColor3 = CurrentColors.Accent
        NoBtn.ZIndex = 502
        NoBtn.Parent = Dialog
        ...
        ConfirmBtn.MouseButton1Click:Connect(function()
            ...
            -- Clean up physical glass + DoF + lighting
            DestroyGlassPart()
            if dof then pcall(function() dof:Destroy() end); dof = nil end
            if dofTrackConn then dofTrackConn:Disconnect(); dofTrackConn = nil end
            ...
            TweenService:Create(MainFrame, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.In),
                {Size = UDim2.new(0, 0, 0, 0)}):Play()
            task.delay(0.38, function() MainFrame.Visible = false end)
    ...
    function Window:Tab(name)
        ...
        TabBtn.MouseButton1Click:Connect(function()
            ...
            Page.Visible = true
            Tween(TabBtn, {BackgroundTransparency = 0.06, TextColor3 = CurrentColors.Text, BackgroundColor3 = CurrentColors.Top})
        ...
        local isSystemTab = (name == "Config" or name == "Liquid Glass")
        if firstTab and not isSystemTab then
            firstTab = false
            Page.Visible = true
            TabBtn.TextColor3 = CurrentColors.Text
            TabBtn.BackgroundTransparency = 0.06
            TabBtn.BackgroundColor3 = CurrentColors.Top
            TabBar.BackgroundTransparency = 0
        end

        if name == "Config"       then TabBtn.LayoutOrder = 99998 end
        if name == "Liquid Glass" then TabBtn.LayoutOrder = 99999 end
        ...
        function Elements:Section(text)
            ...
            S.Parent = Page; S.TextTransparency = 0.35
        ...
        function Elements:Label(text)
            ...
            L.Parent = Page; L.TextTransparency = 0.2
        ...
        function Elements:Paragraph(title, body)
            ...
            BLbl.Parent = F; BLbl.TextTransparency = 0.25
        ...
        local function MakeTile(h)
            ...
            A.BorderSizePixel = 0; A.Parent = F
        ...
        function Elements:Button(text, callback, silent)
            ...
            Btn.MouseEnter:Connect(function() Tween(Btn, {BackgroundTransparency = math.clamp(LiquidGlassConfig.GlassOpacity - 0.04, 0, 1)}, 0.18) end)
            Btn.MouseLeave:Connect(function() Tween(Btn, {BackgroundTransparency = math.clamp(LiquidGlassConfig.GlassOpacity + 0.04, 0, 0.98)}, 0.18) end)

            table.insert(ThemeListeners, function()
                Btn.BackgroundColor3 = CurrentColors.Top
                Btn.TextColor3 = CurrentColors.Text
                Bar.BackgroundColor3 = CurrentColors.Accent
            end)
        end
        ...
        function Elements:Toggle(text, default, callback)
            ...
            Switch.BackgroundColor3 = Enabled and CurrentColors.Accent or CurrentColors.Stroke
            ...
            local function Update()
                Tween(Switch, {BackgroundColor3 = Enabled and CurrentColors.Accent or CurrentColors.Stroke})
        ...
        function Elements:Slider(text, min, max, default, callback)
            ...
            Num.FocusLost:Connect(function()
                ...
                if typed then Update(typed) else Num.Text = tostring(Val) end
            end)
            ...
            table.insert(ThemeListeners, function()
                if Fill then Fill.BackgroundColor3 = CurrentColors.Accent end
                if Track then Track.BackgroundColor3 = CurrentColors.Stroke end
                Num.TextColor3 = CurrentColors.Accent
            end)
        end
        ...
        function Elements:Dropdown(text, options, callback, silent)
            ...
            AddGlassTransparency(Container, 0.04)
            ...
            local function Select(opt)
                ...
                if not silent then InternalNotif(text, opt, "info") end
                callback(opt)
            ...
            local function RefreshOptions(newOpts)
                ...
                for _, opt in pairs(newOpts) do
                    ...
                    O.TextColor3 = CurrentColors.Text
                    table.insert(OptionButtons, O)
                    O.MouseButton1Click:Connect(function() Select(opt) end)
                    O.MouseEnter:Connect(function() Tween(O, {TextColor3 = CurrentColors.Accent}, 0.15) end)
                    O.MouseLeave:Connect(function() Tween(O, {TextColor3 = CurrentColors.Text}, 0.15) end)
                end
            end

            table.insert(ThemeListeners, function()
                for _, O in pairs(OptionButtons) do
                    if O and O.Parent then O.TextColor3 = CurrentColors.Text end
                end
            end)
        ...
        function Elements:Textbox(text, placeholder, callback, silent)
            ...
            Box.FocusLost:Connect(function()
                ...
                if not silent and Box.Text ~= "" then InternalNotif(text, Box.Text, "info") end
                callback(Box.Text)
            end)
        ...
        function Elements:ColorPicker(text, default, callback)
            ...
            AddGlassTransparency(Panel, 0.04)
            ...
            HueBar.BorderSizePixel = 0; HueBar.Parent = Panel
            ...
            local function MakeRGBBox(label, xPos)
                ...
                Prefix.Font = Enum.Font.GothamBold; Prefix.TextSize = 10
                Prefix.TextXAlignment = Enum.TextXAlignment.Left; Prefix.Parent = Holder
                AddToRegistry(Prefix, "TextColor3", "Accent")

                local Box = Instance.new("TextBox")
                Box.Size = UDim2.new(1, -26, 1, 0)
                Box.Position = UDim2.new(0, 22, 0, 0)
                Box.Text = "0"; Box.BackgroundTransparency = 1
                ...
                Box.Focused:Connect(function() Tween(HolderStroke, {Transparency = 0.15}, 0.15) end)
                Box.FocusLost:Connect(function() Tween(HolderStroke, {Transparency = 0.75}, 0.15) end)
    ...
    GlassTab:Slider("Glass Opacity", 0, 100, math.floor(LiquidGlassConfig.GlassOpacity * 100), function(v)
        LiquidGlassConfig.GlassOpacity = v / 100
        ApplyGlassChanges()
    end)

    GlassTab:ColorPicker("Glass Tint", LiquidGlassConfig.GlassTint, function(c)
        LiquidGlassConfig.GlassTint = c
        ApplyGlassChanges()
    end)

    GlassTab:Slider("Refraction Glow", 0, 100, math.floor(LiquidGlassConfig.RefractionIntensity * 100), function(v)
        LiquidGlassConfig.RefractionIntensity = v / 100
        ApplyGlassChanges()
    end)

    -- REFLECTION
    GlassTab:Section("reflection")

    GlassTab:Toggle("Physical Glass Part", LiquidGlassConfig.ReflectionEnabled, function(v)
        LiquidGlassConfig.ReflectionEnabled = v
        ApplyGlassChanges()
    end)

    GlassTab:Slider("Reflection Intensity", 1, 15, LiquidGlassConfig.ReflectionIntensity, function(v)
        LiquidGlassConfig.ReflectionIntensity = v
        ApplyGlassChanges()
    end)

    GlassTab:Slider("Environment Specular", 0, 100, math.floor(LiquidGlassConfig.EnvironmentSpecular * 100), function(v)
        LiquidGlassConfig.EnvironmentSpecular = v / 100
        ApplyGlassChanges()
    end)

    -- DEPTH BLUR
    GlassTab:Section("depth blur")

    GlassTab:Toggle("Depth Blur (DoF)", LiquidGlassConfig.DepthBlurEnabled, function(v)
        LiquidGlassConfig.DepthBlurEnabled = v
        ApplyGlassChanges()
    end)

    GlassTab:Slider("Blur Intensity", 0, 100, math.floor(LiquidGlassConfig.DepthBlurIntensity * 100), function(v)
        LiquidGlassConfig.DepthBlurIntensity = v / 100
        ApplyGlassChanges()
    end)

    GlassTab:Slider("Focus Radius", 1, 50, LiquidGlassConfig.FocusRadius, function(v)
        LiquidGlassConfig.FocusRadius = v
        ApplyGlassChanges()
    end)

    -- LIQUID ANIMATION
    GlassTab:Section("liquid animation")

    GlassTab:Slider("Liquid Speed", 1, 100, math.floor(LiquidGlassConfig.LiquidSpeed * 10), function(v)
        LiquidGlassConfig.LiquidSpeed = v / 10
        ApplyGlassChanges()
    end)

    GlassTab:Slider("Gloss Opacity", 0, 100, math.floor(LiquidGlassConfig.LiquidGlossOpacity * 100), function(v)
        LiquidGlassConfig.LiquidGlossOpacity = v / 100
        ApplyGlassChanges()
    end)

    GlassTab:ColorPicker("Sheen Color", LiquidGlassConfig.SheenColor, function(c)
        LiquidGlassConfig.SheenColor = c
        ApplyGlassChanges()
    end)

    -- RAINBOW EDGE
    GlassTab:Section("rainbow edge")

    GlassTab:Toggle("Rainbow Edge", false, function(v) Library:ToggleRainbow(v) end)

    GlassTab:Slider("Rainbow Speed", 0, 10, 1, function(v)
        Library:SetRainbowSpeed(v)
    end)

    GlassTab:Dropdown("Rainbow Type", {
    ...
    }, function(val) Library:SetRainbowType(val) end, true)

    -- GENERAL
    GlassTab:Section("general")

    GlassTab:Keybind("Menu Keybind", Keybind or Enum.KeyCode.M, function(v) Window:SetKeybind(v) end)

    GlassTab:Button("Unload UI", function() Window:Unload() end, true)
    ...
    do
        for _, r in pairs(Registry) do
            if r.Object and r.Object.Parent then
                pcall(function() r.Object[r.Property] = CurrentColors[r.Type] end)
            end
        end
        for _, fn in pairs(ThemeListeners) do pcall(fn) end
    end
